...
function cloneWorkflow(workflow: LexFrameWorkflow): LexFrameWorkflow {
...
function safePreview(value: string, limit = 160) {
...
function buildSessionPolicySummary(policy: AiPolicyContext) {
  return {
...
function buildSecurityLabels(dataClass: AiDataClass) {
  if (dataClass === 'C_LEGAL_SECRET') {
...
function mapAiDataClassToInputClass(dataClass: AiDataClass) {
  switch (dataClass) {
    case 'C_LEGAL_SECRET':
      return 'legal_secret';
...
function mapDataClassificationToAiDataClass(
...
function summarizeAiResponse(response: AiChatResponse) {
...
function buildDocumentAnalysisDescription(
  documents: readonly DocumentContextRow[],
) {
