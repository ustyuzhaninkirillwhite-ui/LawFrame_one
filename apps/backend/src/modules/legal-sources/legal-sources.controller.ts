...
function parseCreateLegalImportJobRequest(
  body: unknown,
): CreateLegalImportJobRequest {
  const value = asRecord(body);

  return {
    providerCode: expectString(
      value.providerCode,
    ...
    ...(Array.isArray(value.files)
      ? {
          files: value.files.map((entry) => {
            ...
            return {
              uploadId: expectString(
                file.uploadId,
                'File uploadId is required.',
              ),
              title: expectString(file.title, 'File title is required.'),
              mimeType: expectString(
                file.mimeType,
...
function expectInputType(
  value: unknown,
): CreateLegalImportJobRequest['inputType'] {
  if (
    value === 'files' ||
    value === 'document' ||
    value === 'seed' ||
...
function expectClassification(value: unknown): DataClassification {
  if (
    typeof value === 'string' &&
    dataClassification.includes(value as never)
  ) {
