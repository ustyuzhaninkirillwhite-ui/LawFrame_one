export const permissionCodes = [
  "dashboard.view",
  "workspace.read",
  "workspace.update",
  "workspace.switch",
  "workspace.invite",
  "workspace.member.read",
  "workspace.member.update_role",
  "workspace.member.remove",
  "workspace.security.read",
  "workspace.security.manage",
  "session.read",
  "session.revoke",
  "profile.read",
  "profile.update",
  "profile.publish",
  "profile.override_personal",
  "settings.view",
  "settings.profile.update_self",
  "settings.organization.view",
  "settings.organization.update",
  "settings.ai.view",
  "settings.ai.manage_self",
  "settings.ai.manage_workspace",
  "settings.ai.secret.create_self",
  "settings.ai.secret.rotate_self",
  "settings.ai.secret.delete_self",
  "settings.ai.secret.create_workspace",
  "settings.ai.secret.rotate_workspace",
  "settings.ai.secret.delete_workspace",
  "settings.ai.connection.test",
  "settings.ai.diagnostics.view",
  "settings.ai.effective_policy.view",
  "document.read",
  "document.upload",
  "document.generate",
  "document.delete",
  "document.restore",
  "document.template.read",
  "document.template.publish",
  "document.template.map_fields",
  "document.template.manage",
  "document.validation.read",
  "document.validation.resolve",
  "legal_sources.manage",
  "module.manage",
  "legal_search.use",
  "automation.read",
  "automation.install",
  "automation.edit",
  "automation.fork",
  "automation.update_source",
  "automation.approve_external",
  "automation.publish",
  "automation.sync_runtime",
  "automation.submit_publication",
  "automation.run",
  "canvas.view",
  "canvas.edit",
  "canvas.publish",
  "canvas.restore_version",
  "canvas.manage_locks",
  "canvas.view_validation",
  "canvas.view_raw_dsl",
  "canvas.import_runtime",
  "canvas.add_node",
  "canvas.delete_node",
  "canvas.add_edge",
  "canvas.delete_edge",
  "canvas.edit_layout",
  "canvas.edit_node_config",
  "canvas.edit_bindings",
  "canvas.edit_conditions",
  "canvas.edit_error_handlers",
  "canvas.edit_approval_gates",
  "canvas.edit_delivery_steps",
  "canvas.edit_ai_steps",
  "canvas.edit_runtime_mapping",
  "canvas.compile",
  "canvas.sync_runtime",
  "canvas.view_compile_preview",
  "canvas.resolve_sync_conflict",
  "canvas.policy_override",
  "canvas.security_review",
  "canvas.audit_read",
  "canvas.audit_export",
  "canvas.connection_view",
  "canvas.connection_request",
  "canvas.connection_manage",
  "canvas.version.view",
  "canvas.version.compare",
  "canvas.version.download_json",
  "canvas.checkpoint.create",
  "canvas.publish.validate",
  "canvas.version.restore_as_draft",
  "canvas.version.rollback",
  "canvas.runtime.rollback",
  "canvas.version.emergency_disable",
  "canvas.version.view_runtime_projection",
  "canvas.runtime.view",
  "canvas.runtime.pull",
  "canvas.runtime.import_preview",
  "canvas.runtime.import_apply",
  "canvas.runtime.reject_import",
  "canvas.runtime.overwrite",
  "canvas.runtime.resolve_conflict",
  "canvas.runtime.view_technical_diff",
  "canvas.runtime.import_unknown_step",
  "canvas.runtime.import_code_step",
  "canvas.test.validate",
  "canvas.test.step",
  "canvas.test.branch",
  "canvas.test.loop",
  "canvas.test.dry_run",
  "canvas.test.cancel",
  "canvas.test.view_history",
  "canvas.test.view_redacted",
  "canvas.test.view_raw_data",
  "canvas.test.pin_data",
  "canvas.test.create_fixture",
  "canvas.test.edit_fixture",
  "canvas.test.use_real_ai",
  "canvas.test.use_real_documents",
  "canvas.debug",
  "canvas.open_advanced_builder",
  "canvas.ai.use",
  "canvas.ai.explain",
  "canvas.ai.propose_patch",
  "canvas.ai.apply_patch",
  "canvas.ai.configure_step",
  "canvas.ai.fix_validation",
  "canvas.ai.debug_test",
  "canvas.ai.view_raw_context",
  "canvas.ai.use_sensitive_context",
  "canvas.ai.admin_diagnostics",
  "ai.chat.use",
  "ai.workflow.create",
  "ai.workflow.patch",
  "ai.policy.read",
  "ai.policy.manage",
  "legal_rag.use",
  "ai.use_confidential",
  "ai.use_legal_secret",
  "ai.admin.playground",
  "chat.view",
  "chat.create",
  "chat.edit",
  "chat.delete",
  "chat.export",
  "chat.search",
  "chat.manage_project_context",
  "chat.attach_document",
  "chat.view_route_snapshot",
  "chat.view_raw_data",
  "chat.use_legal_secret_context",
  "prompt_template.manage",
  "legal_skill.manage",
  "automation_builder.view",
  "automation_builder.create_intent",
  "automation_builder.plan",
  "automation_builder.answer_clarification",
  "automation_builder.validate",
  "automation_builder.approve_blueprint",
  "automation_builder.reject_blueprint",
  "automation_builder.convert_to_canvas_draft",
  "automation_builder.create_runtime_draft",
  "automation_builder.view_route_snapshot",
  "automation_builder.view_raw_diagnostics",
  "automation_builder.export",
  "automation_builder.manage_prompt_templates",
  "automation_builder.use_legal_secret_context",
  "activepieces.open_builder",
  "activepieces.sync_flow",
  "approval.route.manage",
  "approval.task.read",
  "approval.task.decide",
  "connections.manage",
  "moderation.review",
  "recommendation.read",
  "recommendation.accept",
  "recommendation.manage",
  "secret.read_metadata",
  "secret.rotate",
  "billing.read",
  "billing.manage",
  "audit.read",
  "audit.export",
  "incident.read",
  "incident.manage",
  "compliance.read",
  "compliance.manage",
  "access_review.manage",
  "support.bundle.create",
] as const;

export type PermissionCode = (typeof permissionCodes)[number];

export const roleCodes = [
  "owner",
  "admin",
  "lawyer",
  "assistant",
  "viewer",
  "security_admin",
  "billing_admin",
] as const;

export type RoleCode = (typeof roleCodes)[number];
